...
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/feikai2012/v2ws-tls_bash_onekey/main/ssl_update.sh"
    if [[ $(crontab -l | grep -c "ssl_update.sh") -lt 1 ]]; then
      if [[ "${ID}" == "ol" ]]; then
          ...
          sed -i "/acme.sh/c 0 3 * * 0 bash ${ssl_lupdate_file}" /var/spool/cron/crontabs/root
      fi
    fi
    judge "cron 计划任务更新"
}
...
ExecStart=/www/server/nginx/sbin/nginx -c ${nginx_dir}/conf/nginx.conf
...
tls_type() {
    if [[ -f "/www/server/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
